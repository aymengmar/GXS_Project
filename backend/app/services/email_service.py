...
def send_warehouse_welcome_email(
    *,
    to_email: str,
    full_name: str,
    external_id: str,
    ...
    """Send the warehouse staff welcome email containing login credentials."""
    msg = MIMEMultipart("alternative")
    msg["Subject"] = "Welcome to GXS Delivery — Your Warehouse Account Is Active"
    msg["From"] = settings.EMAIL_FROM
    msg["To"] = to_email
    ...
    plain = (
        f"Hello {full_name},\n\n"
        "Your GXS Delivery warehouse account has been activated.\n\n"
        f"Login Email:        {to_email}\n"
        f"Temporary Password: {temp_password}\n"
        f"Your Staff ID:      {external_id}\n\n"
        f"Log in here: {login_url}\n\n"
        "IMPORTANT: You must change your password the first time you log in.\n\n"
        "Welcome to the team,\n"
        "GXS Delivery"
...
<div style="max-width:520px;margin:40px auto;padding:0 16px 40px;">
  ...
  <div style="background:#0D1B2E;border-radius:12px 12px 0 0;padding:24px 28px;">
    <h1 style="color:#fff;margin:0;font-size:22px;font-weight:700;">GXS Delivery</h1>
    <p style="color:#94a3b8;margin:4px 0 0;font-size:13px;">Warehouse Account Activation</p>
  </div>
  ...
  <div style="background:#fff;padding:28px;border-radius:0 0 12px 12px;
    ...
    <p style="color:#374151;font-size:14px;margin:0 0 24px;">
      Your GXS Delivery warehouse account is now
      <strong style="color:#16a34a;">active</strong>.
      Use the credentials below to log in.
    </p>
    ...
    <div style="margin-bottom:24px;">
      <p style="color:#6b7280;font-size:11px;font-weight:600;text-transform:uppercase;
                letter-spacing:.06em;margin:0 0 4px;">Your Staff ID</p>
      <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;
                  padding:10px 14px;color:#111827;font-size:15px;">
        {external_id}
      </div>
